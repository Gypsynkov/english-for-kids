...
__webpack_require__.e(/*! import() */ 0).then(__webpack_require__.t.bind(null, /*! babel-polyfill */ "./node_modules/babel-polyfill/lib/index.js", 7));
...
var makeSlides = function makeSlides(posterSrc, moviesTittle, moviesBirthdays, moviesRates, filmCode) {
  var posters = [];
  var tittle = [];
  var birthdays = [];
  var rates = [];
  var links = [];
  posters = document.querySelectorAll('.film-img');
  tittle = document.querySelectorAll('.film-tittle');
  birthdays = document.querySelectorAll('.film-year');
  rates = document.querySelectorAll('.film-rate');

  for (var i = 0; i < posterSrc.length; i++) {
    posters[i].src = posterSrc[i];
    tittle[i].setAttribute('href', "https://www.imdb.com/title/".concat(filmCode[i], "/"));
    tittle[i].innerHTML = moviesTittle[i];
    birthdays[i].innerHTML = moviesBirthdays[i];
    rates[i].innerHTML = moviesRates[i];
  }
};

function getRaiding(_x) {
  return _getRaiding.apply(this, arguments);
}

function _getRaiding() {
  _getRaiding = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(prop) {
    var imdbID, url, res, data;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            imdbID = prop.imdbID;
            url = " https://www.omdbapi.com/?i=".concat(imdbID, "&apikey=e0c0f8fc");
            _context.next = 4;
            return fetch(url);
...
function getMoviesPageData(_x2) {
  return _getMoviesPageData.apply(this, arguments);
}

function _getMoviesPageData() {
  _getMoviesPageData = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee2(name) {
    var url, res, data, promises;
    return regeneratorRuntime.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            //  name = await getTranslate(`${document.querySelector('.searchTerm').value}`);
            url = "https://www.omdbapi.com/?s=".concat(name, "&apikey=e0c0f8fc");
            _context2.next = 3;
            return fetch(url);
          ...
          case 6:
            ...
            promises = data.Search.map(getRaiding);
            _context2.next = 11;
            return Promise.all(promises);
          ...
          case 13:
            alert("\u041E\u0448\u0438\u0431\u043A\u0430 ".concat(data.Error));
            return _context2.abrupt("return", data.Error.toLowerCase());

          case 15:
            postersSrc = data.Search.map(function (obj) {
              return obj.Poster;
            });
            moviesTittle = data.Search.map(function (obj) {
              return obj.Title;
            });
            moviesBirthdays = data.Search.map(function (obj) {
              return obj.Year;
            });
            moviesRates = data.Search.map(function (obj) {
              return obj.imdbRating;
            });
            filmCode = data.Search.map(function (obj) {
              return obj.imdbID;
            });
            makeSlides(postersSrc, moviesTittle, moviesBirthdays, moviesRates, filmCode);
...
function getTranslate(_x3) {
  return _getTranslate.apply(this, arguments);
}

function _getTranslate() {
  _getTranslate = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee3(word) {
    var url, res, data;
    return regeneratorRuntime.wrap(function _callee3$(_context3) {
      while (1) {
        switch (_context3.prev = _context3.next) {
          case 0:
            url = " https://translate.yandex.net/api/v1.5/tr.json/translate?key=trnsl.1.1.20191217T173349Z.1868d6c60c3e3142.63330b0f4edc970e425314ae81010dc96a3286ad&text=".concat(word, "&lang=ru-en");
            _context3.next = 3;
            return fetch(url);
          ...
          case 6:
            data = _context3.sent;
            return _context3.abrupt("return", data.text[0]);
...
var language = function language(text) {
  return /[а-я]/i.test(text);
};

var rewriteInput = function rewriteInput(word) {
  language(word);
};

var searchWords = '';
document.querySelector('.searchButton').addEventListener('click', function () {
  getMoviesPageData(document.querySelector('.searchTerm').value);
  document.querySelector('.notification').innerHTML = '';

  if (language(document.querySelector('.searchTerm').value)) {
    document.querySelector('.notification').value = '';
    document.querySelector('.notification').innerHTML = "Showing results for: ".concat(document.querySelector('.searchTerm').value);
  }
});
document.querySelector('.clearButton').addEventListener('click', function () {
  document.querySelector('.notification').innerHTML = '';
  document.querySelector('.searchTerm').value = '';
  var keyboard = new _js_keyboard__WEBPACK_IMPORTED_MODULE_0__["default"]();
...
document.addEventListener("keydown", function (event) {
  if (event.key === "Enter") {
    ...
    getMoviesPageData("".concat(document.querySelector('.searchTerm').value));
  }
});
document.querySelector('.keyBoardButton').addEventListener('click', function () {
  var keyboard = new _js_keyboard__WEBPACK_IMPORTED_MODULE_0__["default"]();
  keyboard.addListenersOnKeys();
  keyboard.init();
  keyboard.addKeysOnKeyboard();
});
document.querySelector('.closekeyBoardButton').addEventListener('click', function () {
  document.querySelector('.keyboard_wrap').classList.add('wrapper_none');
});

window.onload = function () {
  alert('Привет, друг. Из-за проблем со временем, приложение не сделано полностью. Что бы начать работу, введи в поле ввода нужное тебе слово и нажми поиск, тогда слайды появятся. Так-же много багов в слайдере и клавиатуре. Надеюсь на понимаение, спасибо!');
};
...
var Keyboard = /*#__PURE__*/function () {
  function Keyboard() {
    ...
    this.keyWhich = [192, 49, 50, 51, 52, 53, 54, 55, 56, 57, 48, 189, 187, 8, 9, 81, 87, 69, 82, 84, 89, 85, 73, 79, 80, 219, 221, 220, 46, 20, 65, 83, 68, 70, 71, 72, 74, 75, 76, 186, 222, 13, 16, 90, 88, 67, 86, 66, 78, 77, 188, 190, 191, 38, 16, 17, 91, 18, 32, 18, 37, 40, 39, 17];
    this.keysOnEng = ['`', '1', "2", "3", "4", "5", "6", "7", "8", "9", "0", "-", "=", 'Backspace', 'Tab', "q", "w", "e", "r", "t", "y", "u", "i", "o", "p", "[", "]", "\\", 'Delete', 'CapsLock', "a", "s", "d", "f", "g", "h", "j", "k", "l", ";", "'", 'Enter', "Shift", "z", "x", "c", "v", "b", "n", "m", ",", ".", "/", '&uarr;', "Shift", "Control", "Meta", "Alt", "Space", "Alt", "&larr;", "&darr;", "&rarr;", "Control"];
    this.keysOnEngUp = ['~', '!', "@", "#", "$", "%", "^", "&", "*", "(", ")", "_", "+", 'Backspace', 'Tab', "Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P", "{", "}", "|", 'Delete', 'CapsLock', "A", "S", "D", "F", "G", "H", "J", "K", "L", ":", '"', 'Enter', "Shift", "Z", "X", "C", "V", "B", "N", "M", "<", ">", "?", '&uarr;', "Shift", "Control", "Meta", "Alt", "Space", "Alt", "&larr;", "&darr;", "&rarr;", "Control"];
    this.keysOnRuss = ['ё', '1', "2", "3", "4", "5", "6", "7", "8", "9", "0", "-", "=", 'Backspace', 'Tab', "й", "ц", "у", "к", "е", "н", "г", "ш", "щ", "з", "х", "ъ", "\\", 'Delete', 'CapsLock', "ф", "ы", "в", "а", "п", "р", "о", "л", "д", "ж", "э", 'Enter', "Shift", "я", "ч", "с", "м", "и", "т", "ь", "б", "ю", ".", '&uarr;', "Shift", "Control", "Meta", "Alt", "Space", "Alt", "&larr;", "&darr;", "&rarr;", "Control"];
    this.keysOnRussUp = ['Ё', '!', '"', "№", ";", "%", ":", "?", "*", "(", ")", "_", "+", 'Backspace', 'Tab', "Й", "Ц", "У", "К", "Е", "Н", "Г", "Ш", "Щ", "З", "Х", "Ъ", "/", 'Delete', 'CapsLock', "Ф", "Ы", "В", "А", "П", "Р", "О", "Л", "Д", "Ж", "Э", 'Enter', "Shift", "Я", "Ч", "С", "М", "И", "Т", "Ь", "Б", "Ю", ",", '&uarr;', "Shift", "Control", "Meta", "Alt", "Space", "Alt", "&larr;", "&darr;", "&rarr;", "Control"];
  ...
  _createClass(Keyboard, [{
    ...
    value: function init() {
      ...
      var body = document.querySelector('.keyboard_wrap');
      body.appendChild(this.wrapper);
      this.textarea = document.querySelector('.searchTerm');
      this.wrapper.appendChild(this.keyboard);
      this.helper = document.createElement('div');
      this.helper.innerText = 'Close Keyboard';
    ...
    value: function keyDownHandler(event) {
      var elm = document.querySelector('.searchTerm');
      event.preventDefault();
